...
class ArticleDB(Base):
    ...
    currency = Column(String(50), index=True)
    provider_no = Column(String(50), index=True)
    provider = Column(String(50), index=True)
    price = Column(Float)


DATABASE_URL = "sqlite:///./test.db"  # Update with your SQLite database info
...
@app.get("/articles/{article_no}", response_model=Article, tags=["Articles"])
...
@app.post("/articles/", response_model=Article, tags=["Articles"])
...
@app.put("/articles/{article_no}", response_model=Article, tags=["Articles"])
...
@app.delete("/articles/{article_no}", response_model=Article, tags=["Articles"])
